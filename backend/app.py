import cx_Oracle
from flask import Flask, escape, abort, request, render_template, jsonify
...
@app.route('/bloodvis/api')
def index():
    return "Bloodvis API"
    

@app.route('/bloodvis/api/get_attributes', methods=['GET'])
def get_tasks():
...
# Request data with x_axis, y_axis, year_range, filter_option
# x_axis and y_axis are the name of the columns that we are retrieving
# x_axis can be year/surgeon/anesthologist
# y_axis is blood products
# year_range is the limit on the year
# filter_section is a list of procedure that we are limiting the data on
@app.route('/bloodvis/api/requestwithyear/', methods=['GET'])
def create_with_year():
...
#NOT USED
#This is actually not called at the moment
#Returns the the corresponding x_axis and y_axis value
#where x_axis is year/surgeon/ANESTHOLOGIST
#y_axis is the blood products
@app.route('/bloodvis/api/tasks',methods = ['GET'])
def create_task():
    x_axis = request.args.get('x_axis')
    y_axis = request.args.get('y_axis')
    ...
    #possible input [YEAR, SURGEON_ID, ANESTHOLOGIST_ID, HEMOGLOBIN, PRBC_UNITS, FFP_UNITS, PLT_UNITS, CRYO_UNITS]
    command_dict = {
        'YEAR': 'EXTRACT (YEAR FROM CLIN_DM.BPU_CTS_DI_INTRAOP_TRNSFSD.DI_TRNSFSN_DTM)',
        'SURGEON_ID': 'CLIN_DM.BPU_CTS_DI_SURGERY_CASE.SURGEON_PROV_DWID',
        'ANESTHOLOGIST_ID': 'CLIN_DM.BPU_CTS_DI_SURGERY_CASE.ANESTH_PROV_DWID',
        'PRBC_UNITS' : 'SUM(CLIN_DM.BPU_CTS_DI_INTRAOP_TRNSFSD.PRBC_UNITS)',
        'FFP_UNITS':'SUM(CLIN_DM.BPU_CTS_DI_INTRAOP_TRNSFSD.FFP_UNITS)',
    ...
    data_origin = {
        'YEAR': 'CLIN_DM.BPU_CTS_DI_INTRAOP_TRNSFSD', 
        'PRBC_UNITS': 'CLIN_DM.BPU_CTS_DI_INTRAOP_TRNSFSD',
        'FFP_UNITS': 'CLIN_DM.BPU_CTS_DI_INTRAOP_TRNSFSD', 
        'PLT_UNITS': 'CLIN_DM.BPU_CTS_DI_INTRAOP_TRNSFSD',
        'CRYO_UNITS': 'CLIN_DM.BPU_CTS_DI_INTRAOP_TRNSFSD', 
        'SURGEON_ID': 'CLIN_DM.BPU_CTS_DI_SURGERY_CASE',
        'ANESTHOLOGIST_ID':'CLIN_DM.BPU_CTS_DI_SURGERY_CASE', 
        'Hemoglobin':'CLIN_DM.BPU_CTS_DI_PREOP_LABS'
    }
    dict_for_exchange={
        'YEAR': 'EXTRACT (YEAR FROM CLIN_DM.BPU_CTS_DI_INTRAOP_TRNSFSD.DI_TRNSFSN_DTM)',
        'SURGEON_ID': 'SURGEON_PROV_DWID',
    ...
    #data_origin['Hemoglobin':"select * from (\
                    # select DI_PAT_ID, DI_VISIT_NO, DI_RESULT_DTM, RESULT_VALUE, \
                    # row_number() over(partition by DI_VISIT_NO order by DI_RESULT_DTM desc) as rn \
                    # from CLIN_DM.BPU_CTS_DI_PREOP_LABS \
                    # where RESULT_DESC = 'Hemoglobin') t \
                    # where t.rn = 1"]
    # if x_axis == 'Hemoglobin' or y_axis == 'Hemoglobin':
            ...
            #         where RESULT_DESC = 'Hemoglobin') t \
            #         where t.rn = 1 "
            # if x_axis == 'Hemoglobin':
            #     command = 'SELECT RESULT_VALUE, ' + command_dict[y_axis] + ''
    # else:
    if data_origin[x_axis] == data_origin[y_axis]:
        command = 'SELECT ' + command_dict[x_axis] + ', ' + command_dict[y_axis] + ' FROM '+data_origin[x_axis] +' GROUP BY ' + command_dict[x_axis] + ' ORDER BY ' + command_dict[x_axis]
    else:
        command = 'select ' + command_dict[x_axis] + ', ' + command_dict[y_axis] + ' FROM ' + data_origin[x_axis] + ' INNER JOIN ' + data_origin[y_axis] +' ON ' + data_origin[x_axis] + '.DI_VISIT_NO = ' + data_origin[y_axis] +'.DI_VISIT_NO GROUP BY ' + command_dict[x_axis]
    cur = connection.cursor()
    result = cur.execute(command)
    data_exchange = {dict_for_exchange[x_axis].replace(" ",''):'x_axis', dict_for_exchange[y_axis].replace(" ",''):'y_axis'}

    data = [dict(zip([data_exchange[key[0]] for key in cur.description], row)) for row in result]
    return json.dumps({'task':data})


@app.route('/bloodvis/api/hemoglobin', methods = ['GET'])
def hemoglobin():
    command = \
