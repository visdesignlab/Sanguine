...
def recreate_materialized_view_procs(apps, schema_editor):
    ...
    BEGIN
        ...
        SELECT
            ...
            CASE WHEN COALESCE(ap.attend_prov_line, 0) = 1 THEN (CASE WHEN bc_outcomes.stroke = 1 THEN TRUE ELSE FALSE END) ELSE NULL END AS stroke,
            CASE WHEN COALESCE(ap.attend_prov_line, 0) = 1 THEN (CASE WHEN bc_outcomes.ecmo = 1 THEN TRUE ELSE FALSE END) ELSE NULL END AS ecmo,
        ...
        ) pt ON ap.visit_no = pt.visit_no AND COALESCE(ap.attend_prov_line, 0) = pt.attend_prov_line

        LEFT JOIN GuidelineAdherence ga 
            ON ap.visit_no = ga.visit_no AND COALESCE(ap.attend_prov_line, 0) = ga.attend_prov_line
