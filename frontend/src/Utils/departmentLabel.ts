 ...
 *
 * @param departmentIds - The currently-filtered department IDs (from store.filterValues.departmentIds)
...
export function formatDepartmentFilter(
  ...
  hierarchy: DepartmentHierarchyResponse | null,
  preposition: string = 'Within',
): string | null {
  if (!departmentIds.length || !hierarchy) return null;
