...
const BarChartVisualization: FC<Props> = ({ aggregatedBy, valueToVisualize, chartId, store, chartIndex, extraPair }: Props) => {
  ...
  async function fetchChartData() {
    const res = await fetch(
      `http://localhost:8000/api/request_transfused_units?aggregated_by=${aggregatedBy}&transfusion_type=${valueToVisualize}&year_range=${actualYearRange}&filter_selection=${filterSelection.toString()}`
    );
    ...
    if (dataResult) {
      ...
      const caseList = dataResult.case_id;
      ...
      let cast_data = (dataResult as any).map(function (ob: any) {
        ...
        const total_val = sum(ob.transfused_units);
        const medianVal = median(ob.transfused_units);
        let pd = createpd(ob.transfused_units, { min: 0.00001 });
        ...
        const maxY = parseFloat(max(ob.transfused_units)!);
