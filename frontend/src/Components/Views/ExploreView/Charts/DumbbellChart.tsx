...
export function DumbbellChart({ chartConfig }: { chartConfig: DumbbellChartConfig }) {
  ...
  // Handlers
  const toggleProviderCollapse = (e: React.MouseEvent, providerId: string) => {
    e.stopPropagation();
  ...
  const toggleVisitCollapse = (e: React.MouseEvent, uniqueVisitId: string) => {
    e.stopPropagation();
    const next = new Set(collapsedVisits);
    if (next.has(uniqueVisitId)) next.delete(uniqueVisitId);
    else next.add(uniqueVisitId);
    setCollapsedVisits(next);
  };

  const toggleProviderSort = (providerId: string) => {
    const next = new Map(providerSorts);
    next.set(providerId, getNextSortState(next.get(providerId)));
    setProviderSorts(next);
  };

  const toggleVisitSort = (uniqueVisitId: string) => {
    const next = new Map(visitSorts);
  ...
  const processedData = useMemo(() => {
    ...
    groupedByProvider.forEach((cases, providerId) => {
      ...
      let visits = Array.from(groupedByVisit.entries()).map(([visitLabel, visitCases]) => {
                ...
                return (
                  <g key={provider.id}>
                    ...
                    <rect
                      ...
                      onClick={() => toggleProviderSort(provider.id)}
                    />
                    ...
                    <rect
                      ...
                      onClick={(e) => toggleProviderCollapse(e, provider.id)}
                    />
                            ...
                            return (
                              <g key={visit.id}>
                                ...
                                <rect
                                  ...
                                  strokeWidth={1}
                                  style={{ cursor: 'pointer' }}
                                  onClick={() => toggleVisitSort(visit.id)}
                                />
                                <text
                                ...
                                <rect
                                  ...
                                  onMouseLeave={() => setHoveredCollapse(null)}
                                  onClick={(e) => toggleVisitCollapse(e, visit.id)}
                                />
