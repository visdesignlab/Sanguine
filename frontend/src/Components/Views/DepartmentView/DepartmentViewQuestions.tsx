...
export function DepartmentViewQuestions() {
  ...
  const resolveQuestion = useCallback((q: string) => {
    ...
    const deptName = store.departmentHierarchy?.departments.find((d) => d.id === store.selectedDepartmentId)?.name || 'Department';
    return q.replace('[department]', deptName);
  }, [store.departmentHierarchy, store.selectedDepartmentId]);
