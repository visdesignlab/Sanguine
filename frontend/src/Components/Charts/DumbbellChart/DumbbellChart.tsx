...
import { DumbbellLine, StandardLine } from "../../../Presets/StyledSVGComponents";
  ...
  return (
    <>
      ...
      <g className="chart-comp" >
        ...
        <StandardLine x1={currentOffset.left} x2={dimensionWidth - currentOffset.right} y1={testValueScale()(HGB_HIGH_STANDARD)} y2={testValueScale()(HGB_HIGH_STANDARD)} />
        <StandardLine x1={currentOffset.left} x2={dimensionWidth - currentOffset.right} y1={testValueScale()(HGB_LOW_STANDARD)} y2={testValueScale()(HGB_LOW_STANDARD)} />
        ...
        {numberList.map((numberOb, ind) => {
          if (Object.keys(averageForEachTransfused).length > 0) {
            ...
            if (ind >= 1 && numberOb.num <= numberList[ind - 1].num) {
              interventionLine = <StandardLine x1={x1 - 0.5 * (x1 - interval)} x2={x1 - 0.5 * (x1 - interval)} y1={currentOffset.top} y2={dimensionHeight - currentOffset.bottom} />;
            }
            if (x1 && x2) {
              return ([
