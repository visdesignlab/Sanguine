...
export function FilterHistogramWithSliderComponent({
  data,
  unitName,
}: {
  unitName: BloodComponent | 'los';
  data: HistogramData;
}) {
  ...
  function rangeChanged(key: BloodComponent | 'los') {
  ...
  const barHeightScale = useCallback(
    () => scaleLinear([0, maxCountExcludeZeroUnit], [0, svgHeight]).clamp(true),
    [maxCountExcludeZeroUnit],
  );
