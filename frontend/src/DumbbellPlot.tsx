...
class DumbbellPlot extends Component<
    DumbbellPlotProps,
    DumbbellPlotState
    > {
        ...
        fetch(`http://localhost:8000/api/hemoglobin`,
            ...
            .then(data => {
                ...
                if (data) {
                    ...
                    this.setState({
                        data: cast_data, y_max: y_max, x_max: x_max, x_min: x_min
                    },
                        this.drawChart);
                }
            });
    ...
    drawChart() {
        ...
        let x_scale: any;
        x_scale = d3.scaleLinear().domain([0.9 * this.state.x_min, 1.1 * this.state.x_max]).range([offset.left, width])
        const circle_tooltip = svg.select(".circle-tooltip")
        let components = svg.select('#all-dots').selectAll('g').data(data)
        components.exit().remove();
        components = (components as any).enter().append('g').merge(components as any);
