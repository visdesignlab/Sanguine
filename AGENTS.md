...
On top of the data we're provided with, we also generate additional features and metrics that are relevant to PBM. For example, we may calculate transfusion rates, anemia management practices, and blood conservation techniques based on the raw data. These additional features are stored in SQL-managed derived tables in the database, allowing for fast access and efficient querying when generating visualizations and reports. The SQL for these derived artifacts lives in `backend/api/models_derived/`.

Django API
Our backend is built using Django. Since the scale of our data is so large, it's impossible to enable real-time querying of the MariaDB database. Instead, we maintain a set of SQL-managed derived tables that are refreshed and then saved from our Django server into parquet files. These parquet files are then served to the frontend, where we use DuckDB WASM to query and process the data in the browser. This architecture allows us to provide a responsive user experience while still working with large datasets.
...
When working with this codebase, work only with the source code files available to you. If you need an external library, please ask for approval first (and include how well used the library is). Make sure to follow best practices for React and TypeScript development, including proper state management, component structuring, and code documentation. Pay extra attention to lifecycle methods and hooks to ensure optimal performance and avoid memory leaks, including any updates to existing code. If you encounter any issues or have suggestions for improvements, feel free to bring them up for discussion. Don't interact with GitHub directly; instead, focus on the code itself. Always check package.json for the scripts available to you for building, testing, and running the project. Never run backend poetry command (including install and testing), if they're required ask me to. The backend runs inside docker and you cannot access it. Your goal should be to reduce complexity, improve readability, and ensure that the code is maintainable and scalable for future development. That means when fixing a bug, first consider what code can be removed or simplified, rather than just adding more code to patch the issue. When adding a new feature, consider how it fits into the existing architecture and whether it can be implemented in a way that minimizes disruption to the existing codebase. When you modify Django-managed schema, update the corresponding database migrations. When you modify derived artifacts such as `GuidelineAdherence`, `VisitAttributes`, or `SurgeryCaseAttributes`, update the SQL in `backend/api/models_derived/`, ensure `migrate_derived_tables` / `refresh_derived_tables` still reflect the intended workflow, and update the tests and parquet expectations as needed.

LLM system prompt sync:
  - `backend/api/prompts/application_state_system_prompt.md` contains a snapshot of the `ApplicationState` TypeScript types used as the LLM system prompt.
  - When you modify `ApplicationState` or any of its subtypes in `frontend/src/Store/Store.ts` or `frontend/src/Types/application.ts`, you MUST also update `backend/api/prompts/application_state_system_prompt.md` to keep the LLM prompt in sync.
  - The prompt should remain concise — only the type shapes and valid value lists, without extensive prose or code references.

Derived artifact notes:
  - `GuidelineAdherence`, `VisitAttributes`, and `SurgeryCaseAttributes` are SQL-managed derived tables, not Django models.
  - Use `python manage.py migrate_derived_tables` to create or replace their schema.
  - Use `python manage.py refresh_derived_tables` to truncate and repopulate them.
  - `generate_parquets` refreshes the derived tables it needs automatically before reading them.
